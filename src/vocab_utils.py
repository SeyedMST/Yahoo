...
class Vocab(object):
    ...
    def fromVocabualry(self, voc, dim=300):
...
#         self.word_vecs = None
    ...
    def fromMap(self, word2id, word_vecs, word_dim=300):
...
#         self.vocab_size, self.word_dim = map(int, header.split())
        ...
        for line in vec_file:
            ...
            if tmp_dim == -1:
                tmp_dim = self.word_dim
            if self.word_dim != tmp_dim : continue
            if (voc is not None) and (word not in voc): continue
            vector = np.array(parts[1:], dtype='float32')
            cur_index = len(self.word2id)
    ...
    def getVector(self, word):
        if(word in self.word2id):#self.word2id.has_key(word)):
...
if __name__ == '__main__':
    ...
    l = ['yahooqa/', 'wikiqa/WikiQACorpus/WikiQA-']
    sentence_path_list = []
    for qa_path in l:
